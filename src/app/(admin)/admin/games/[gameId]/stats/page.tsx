...
import type { BattingStatsEntry } from '@/components/admin/BattingStatsForm';
...
import type { GameWithTeams, BattingStats, PitchingStats, FieldPosition } from '@/types';
 ...
 * Stats Entry Page
 *
 * Full stats entry interface for a specific game.
...
export default function StatsEntryPage() {
  ...
  // Existing stats (would come from API)
  const [homeBattingStats, setHomeBattingStats] = useState<BattingStats[]>([]);
  const [awayBattingStats, setAwayBattingStats] = useState<BattingStats[]>([]);
  ...
  // Save batting stats handler
  const handleSaveBattingStats = useCallback(
    async (entries: BattingStatsEntry[], teamType: 'home' | 'away') => {
      setIsSaving(true);
      try {
        // Simulate API call
        await new Promise((resolve) => setTimeout(resolve, 1000));

        // Transform entries to BattingStats
        const stats: BattingStats[] = entries.map((entry) => ({
          id: `${gameId}-${entry.playerId}-batting`,
          gameId,
          playerId: entry.playerId,
          teamId: teamType === 'home' ? game!.homeTeamId : game!.awayTeamId,
          battingOrder: entry.battingOrder ?? null,
          positionPlayed: entry.positionPlayed ?? null,
          plateAppearances: entry.plateAppearances ?? 0,
          atBats: entry.atBats ?? 0,
          runs: entry.runs ?? 0,
          hits: entry.hits ?? 0,
          doubles: entry.doubles ?? 0,
          triples: entry.triples ?? 0,
          homeRuns: entry.homeRuns ?? 0,
          runsBattedIn: entry.runsBattedIn ?? 0,
          walks: entry.walks ?? 0,
          strikeouts: entry.strikeouts ?? 0,
          hitByPitch: entry.hitByPitch ?? 0,
          sacrificeFlies: entry.sacrificeFlies ?? 0,
          sacrificeBunts: entry.sacrificeBunts ?? 0,
          stolenBases: entry.stolenBases ?? 0,
          caughtStealing: entry.caughtStealing ?? 0,
          groundIntoDoublePlays: entry.groundIntoDoublePlays ?? 0,
          leftOnBase: entry.leftOnBase ?? 0,
  ...
  return (
    <div className="space-y-6">
      {/* Game Header */}
      <StatsEntryHeader game={game} hasUnsavedChanges={hasUnsavedChanges} />
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {getTeamStatsStatus('away') === 'complete' && getTeamStatsStatus('home') === 'complete'
